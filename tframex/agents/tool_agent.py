...
class ToolAgent(BaseAgent):
    ...
    def __init__(self, 
                 agent_id: str, 
                 tools: List[Tool], # Will be populated by TFrameXApp based on @app.agent(tools=["tool_name"])
        ...
        if target_tool_name_from_config:
            if not tools:
                 raise ValueError(f"ToolAgent '{agent_id}' configured with target_tool_name '{target_tool_name_from_config}' but no tools list was provided.")
            for t in tools:
                if t.name == target_tool_name_from_config:
                    self.tool_to_run = t
                    break
            if not self.tool_to_run:
                raise ValueError(f"ToolAgent '{agent_id}': Tool '{target_tool_name_from_config}' specified in config not found in the agent's tool list: {[t.name for t in tools]}.")
        ...
        super().__init__(agent_id, llm=None, tools=effective_tools_for_base, memory=None, system_prompt_template=None, **config)
    ...
    async def run(self, input_message: Union[str, Message, Dict[str, Any]], **kwargs: Any) -> Message:
        ...
        if isinstance(input_message, dict):
            ...
            if input_message.content is None:
                 logger.error(f"ToolAgent '{self.agent_id}': Received Message with None content.")
                 return Message(role="assistant", name=self.tool_to_run.name, content="Error: Input Message has no content.")
            try:
                input_args = json.loads(input_message.content)
            except json.JSONDecodeError as e:
                logger.error(f"ToolAgent '{self.agent_id}': Invalid JSON in Message content: '{input_message.content}'. Error: {e}")
                return Message(role="assistant", name=self.tool_to_run.name, content=f"Error: Invalid JSON input for ToolAgent '{self.agent_id}'. Content was not valid JSON.")
        elif isinstance(input_message, str):
            try:
                input_args = json.loads(input_message)
            except json.JSONDecodeError as e:
                logger.error(f"ToolAgent '{self.agent_id}': Invalid JSON string input: '{input_message}'. Error: {e}")
                return Message(role="assistant", name=self.tool_to_run.name, content=f"Error: Invalid JSON string input for ToolAgent '{self.agent_id}'. Expected a JSON string of arguments.")
        else:
            return Message(role="assistant", name=self.tool_to_run.name, content=f"Error: Invalid input type for ToolAgent '{self.agent_id}'. Expected JSON string, dict, or Message with JSON content.")

        # Tool.execute expects a JSON string of arguments.
        args_json_str_for_tool = json.dumps(input_args)

        logger.info(f"ToolAgent '{self.agent_id}' executing tool '{self.tool_to_run.name}' with JSON args: {args_json_str_for_tool[:200]}")
        try:
            ...
            elif isinstance(tool_result, (dict, list)): # If result is complex, stringify as JSON
                result_content = json.dumps(tool_result)
            else: # Otherwise, simple string conversion
                result_content = str(tool_result)
            
            return Message(role="assistant", name=self.tool_to_run.name, content=result_content)
        except Exception as e:
            logger.error(f"ToolAgent '{self.agent_id}' error running tool '{self.tool_to_run.name}': {e}", exc_info=True)
            return Message(role="assistant", name=self.tool_to_run.name, content=f"Error executing tool '{self.tool_to_run.name}': {str(e)}")