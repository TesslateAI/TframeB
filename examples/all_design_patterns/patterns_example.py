...
special_llm_config = OpenAIChatLLM(
    ...
    api_base_url=os.getenv("OPENAI_API_BASE_SPECIAL", os.getenv("OPENAI_API_BASE", "http://localhost:11434")),
...
if not special_llm_config.api_base_url: # Check if you intend to use a special LLM
    print("Warning: OPENAI_API_BASE_SPECIAL not set for special LLM. It will use the default base if not overridden.")
...
async def get_current_weather(location: str, unit: str = "celsius") -> str:
    ...
    if "tokyo" in location.lower(): return f"The current weather in Tokyo is 25°{unit.upper()[0]} and sunny."
    if "paris" in location.lower(): return f"The current weather in Paris is 18°{unit.upper()[0]} and cloudy."
...
async def get_city_info(city_name: str, info_type: str = "population") -> str:
    ...
    if "paris" in city_name.lower():
        ...
        if info_type == "attractions": return "Main attractions: Eiffel Tower, Louvre Museum."
    if "tokyo" in city_name.lower():
        if info_type == "population": return "Population of Tokyo is approximately 14 million."
        if info_type == "attractions": return "Main attractions: Tokyo Skytree, Senso-ji Temple."
    return f"Information of type '{info_type}' for {city_name} not found."

# --- Agent Definitions ---

# Basic Agents
@app.agent(name="EchoAgent", description="Repeats the input.", system_prompt="Repeat the user's message verbatim.")
async def echo_agent_placeholder(): pass

@app.agent(name="UpperCaseAgent", description="Converts input to uppercase.", system_prompt="Convert user's message to uppercase. ONLY respond with uppercased text.")
async def uppercase_agent_placeholder(): pass

@app.agent(name="ReverseAgent", description="Reverses the input text.", system_prompt="Reverse the text of user's message. ONLY respond with reversed text.")
...
@app.agent(
    ...
    strip_think_tags=True    # Will remove <think>...</think> from the final output
)
async def creative_writer_placeholder(): pass

# Agents for Discussion Pattern
@app.agent(name="OptimistAgent", description="Optimistic discussant.", system_prompt="You are the Optimist. Find positive aspects. Start with 'As an optimist, I see that...'")
async def optimist_placeholder(): pass

@app.agent(name="PessimistAgent", description="Pessimistic discussant.", system_prompt="You are the Pessimist. Point out downsides. Start with 'However, from a pessimistic view, ...'")
async def pessimist_placeholder(): pass

@app.agent(name="RealistAgent", description="Realistic discussant.", system_prompt="You are the Realist. Provide a balanced view. Start with 'Realistically speaking, ...'")
async def realist_placeholder(): pass

@app.agent(name="DiscussionModeratorAgent", description="Moderates discussions.", system_prompt="Summarize discussion round, identify themes, pose follow-up question.")
...
# 1. Sequential Flow
sequential_flow = Flow(flow_name="SequentialEchoUpperReverse", description="Echoes, uppercases, then reverses input.")
sequential_flow.add_step("EchoAgent").add_step("UpperCaseAgent").add_step("ReverseAgent")
app.register_flow(sequential_flow)

# 2. Parallel Flow
parallel_flow = Flow(flow_name="ParallelWeatherCityInfoSummarize", description="Gets weather & city info in parallel, then summarizes.")
parallel_flow.add_step(ParallelPattern(pattern_name="GetInfoInParallel", tasks=["WeatherAgent", "CityInfoAgent"]))
parallel_flow.add_step("SummarizerAgent")
app.register_flow(parallel_flow)

# 3. Router Flow (Old method)
router_flow_old = Flow(flow_name="RouterFlow_OldMethod", description="Routes task using TaskRouterAgent.")
router_flow_old.add_step(RouterPattern(
    pattern_name="MainTaskRouter", router_agent_name="TaskRouterAgent",
    routes={"weather": "WeatherAgent", "city_info": "CityInfoAgent", "echo": "EchoAgent"},
    default_route="EchoAgent"
))
app.register_flow(router_flow_old)

# 4. Discussion Flow
discussion_flow_example = Flow(flow_name="TeamDebateFlow", description="Optimist, Pessimist, Realist discuss a topic.")
discussion_flow_example.add_step(DiscussionPattern(
    pattern_name="TeamDebateOnTopic",
    participant_agent_names=["OptimistAgent", "PessimistAgent", "RealistAgent"],
    discussion_rounds=2, moderator_agent_name="DiscussionModeratorAgent", stop_phrase="end discussion now"
))
app.register_flow(discussion_flow_example)

# 5. NEW: Flow using Agent-as-Tool (Supervisor Agent)
smart_delegate_flow = Flow(flow_name="SmartDelegateFlow_NewMethod", description="Uses SmartQueryDelegateAgent.")
...
creative_writing_flow = Flow(flow_name="CreativeWriterFlow", description="Demonstrates specialized LLM and think tag stripping.")
creative_writing_flow.add_step("CreativeWriterAgent") # This agent has its own LLM and strip_think_tags=True
...
@app.agent(name="GreetingAgent", description="Greets a user by name.", system_prompt="User's name: {user_name}. Greet them for query: '{user_query}'.")
async def greeting_agent_placeholder(): pass
templated_flow_example = Flow(flow_name="TemplatedGreetingFlow", description="Greets a user by name via template.")
...
async def main():
    async with app.run_context(llm_override=None) as rt: # Can override context LLM here if needed
...
if __name__ == "__main__":
    if not default_llm_config.api_base_url: # Check the default LLM used by TFrameXApp
        print("FATAL: OPENAI_API_BASE environment variable is not set for the default LLM.")
    else:
        asyncio.run(main())